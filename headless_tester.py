...
def open_link(url, driver, toggles):
	...
	if ("ErrorException" in driver.page_source) or ("Error" in driver.page_source):
		...
		print(f"{option_url[37:]} has failed us")
		...
		failures[url] = "error"
		with open(f"{toggles['txt_folder']}{url[37:]}_error.txt", "a") as f:
			f.write(f"{url} \n\n")
	...
	#driver.get(url)
		if toggles['gather_load_time'] == True:
			with open(f"{toggles['txt_folder']}success_data.txt", "a") as f:
...
def test_all_links(driver, toggles):
...
#
	...
	toggles = {'error':True, 'fatal':True, 'warn':True,'anything_other_than_info':True, 'info':False, 'all_console': False, 'error_page_only': True, 'debug_script': False, 'gather_load_time': False, 'png_folder': 'screenshots\\', 'txt_folder': 'logs\\'}
